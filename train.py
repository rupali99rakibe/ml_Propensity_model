...
def train_model(final_data):
    # Training, Test, & Split
    X = final_data.drop("deposit", axis = 1) # input features
    y = final_data["deposit"] # output feature
    
    # one hot categorical variable
    X = pd.get_dummies(X, drop_first=True)
    
    X_train , X_test , y_train , y_test = train_test_split(X, y, test_size = 0.3, random_state = 42, stratify=y)

    # Build the final model using the best-obtained parameters from our experiment
